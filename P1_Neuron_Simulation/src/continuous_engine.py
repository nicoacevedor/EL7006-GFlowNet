...
from .gflownet_functions import reward_function, get_parents_flow_continuous


torch.autograd.set_detect_anomaly(True)

class GFlowContinuousEngine:
    ...
    def fit(
        ...
        for epoch in tqdm(range(n_epochs), desc="Training..."):
            ...
            for batch in pbar:
                ...
                opt.step()
                opt.zero_grad()
        self.save_training("training/continuous/exp1")
    ...
    def sample_matrix(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for t in range(matrix_length):
            ...
            new_state[t] = flow_prediction[action]
            parents_flow = get_parents_flow_continuous(new_state, t)
            if t == matrix_length - 1:
                x_hat = self.neuron_simulator.simulate_neurons(
                    A=new_state.detach().reshape(n_neurons, n_neurons),
                    timesteps=x.shape[1],
                    initial_value=x[:, 0]
                )
    ...
    def save_n_samples(self, n_samples: int, dim: tuple[int, int], path: Path) -> None:
        ...
        for i, matrix in enumerate(sampled_matrices):
            ...
            plt.imshow(matrix.detach().reshape(3, 3).cpu(), cmap="coolwarm")
            plt.axis("off")
